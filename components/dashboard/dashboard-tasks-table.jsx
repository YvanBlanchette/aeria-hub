						...
						>
							...
							<TableCell>
								<div>
									...
									<p className="text-xs text-muted-foreground">{overdue ? "Needs attention" : "Open task"}</p>
								</div>
							</TableCell>
							<TableCell className="text-muted-foreground">
								<Link
									href={`/trips/${task.tripId}/tasks`}
									className="hover:underline"
								>
									{task.tripName}
								</Link>
							</TableCell>
							<TableCell className={cn("text-right", overdue && "text-destructive")}>{task.dueDate ? formatDate(task.dueDate) : "—"}</TableCell>
						</TableRow>
					);
				})}
			</TableBody>
