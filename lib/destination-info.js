...
async function getCruisePortFromDb(location) {
	...
	const candidates = await prisma.cruisePort.findMany({
		where: { OR: [{ name: { contains: primaryPlaceName(location), mode: "insensitive" } }, { displayText: { contains: primaryPlaceName(location), mode: "insensitive" } }] },
		select: { name: true, displayText: true, country: true, latitude: true, longitude: true, description: true, gettingFromPort: true, thingsToDo: true, sourceUrl: true },
		take: 20,
	});
