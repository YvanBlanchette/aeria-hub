...
export async function createTrip(prevState, formData) {
	...
	if (!fields.name || !fields.destination) return t("errors.tripNameDestinationRequired", "Trip name and destination are required.");
	if (fields.startDate && fields.endDate && fields.endDate < fields.startDate) {
		return t("errors.tripEndBeforeStart", "End date can't be before the start date.");
	}
...
export async function duplicateTrip(tripId, prevState, formData) {
	...
	if (!source) return t("errors.tripNotFound", "Trip not found.");
...
export async function addTripClient(tripId, prevState, formData) {
	...
	if (trip.clientId === clientId) return t("errors.tripClientAlreadyPrimary", "This client is already the primary client on this trip.");

	const existing = await prisma.tripClient.findUnique({
		where: { tripId_clientId: { tripId, clientId } },
	});
	if (existing) return t("errors.tripClientAlreadyAdded", "This client is already on this trip.");
