...
export default async function ClientTripsPage({ params }) {
	...
	const trips = [...primaryTrips.map((trip) => ({ ...trip, isCompanion: false })), ...companionLinks.map(({ trip }) => ({ ...trip, isCompanion: true }))].sort(
	...
	return (
		<div className="space-y-4">
			...
			{trips.length === 0 ? (
				<Card>
					<CardContent className="py-10 text-center text-sm text-muted-foreground">No trips on record for this client.</CardContent>
				</Card>
			) : (
							...
							<Card className="transition-colors hover:bg-muted/40">
								<CardContent className="flex flex-wrap items-center justify-between gap-2 p-4">
									...
									<div className="flex items-center gap-3">
										{trip.totalPrice != null && <span className="text-sm tabular-nums text-muted-foreground">{formatCurrency(trip.totalPrice)}</span>}
