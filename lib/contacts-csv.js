...
/**
 * Detects the export format from its most distinctive columns rather than
 * the name columns — Google switched its name columns from "Given
 * Name"/"Family Name" to "First Name"/"Last Name" (matching Outlook) in a
 * 2023-ish CSV export update, so the name columns alone can't tell the
 * formats apart anymore. Google's numbered "<Field> N - Value" convention
 * for email/phone/address is the reliable signature.
 * @param {string[]} headers
 */
export function detectCsvFormat(headers) {
  const set = new Set(headers);
  if (set.has("E-mail 1 - Value") || set.has("Phone 1 - Value") || set.has("Address 1 - Street")) {
    return "google";
  }
  if (set.has("E-mail Address") || set.has("Home Street") || set.has("Business Street")) {
    return "outlook";
  }
  // Fall back to name columns for minimal exports with no contact details at all.
...
export function mapRowToClient(row, format) {
  ...
  if (format === "google") {
    const name = get("Name");
    return {
      firstName: get("Given Name") || get("First Name") || name?.split(" ")[0] || "",
      lastName: get("Family Name") || get("Last Name") || (name ? name.split(" ").slice(1).join(" ") : "") || "",
