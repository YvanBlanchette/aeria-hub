...
export default async function TripCommissionsPage({ params }) {
	...
	return (
		<div className="space-y-4">
			...
			{segments.length === 0 ? (
				<p className="text-sm text-muted-foreground">No commissions set yet. Add one from a segment on the Itinerary tab.</p>
			) : (
							...
							<Card key={segment.id}>
								<CardContent className="space-y-1 p-4">
									<div className="flex flex-wrap items-center gap-2">
										...
										{segment.supplier && <span className="text-sm text-muted-foreground">· {segment.supplier.name}</span>}
									</div>
									<SegmentCommission
										segment={segment}
										tripId={tripId}
