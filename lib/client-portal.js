...
import { getInvoiceOutstandingBalance, getInvoicePaidAmount } from "@/lib/invoices";

export async function resolveClientPortalContext(user) {
	if (!user || user.role !== "CLIENT") return null;

	const normalizedEmail = (user.email || "").trim().toLowerCase();
	if (!user.id && !normalizedEmail) return null;

	const client = await prisma.client.findFirst({
		where: {
			OR: [
				...(user.id ? [{ portalUser: { id: user.id } }] : []),
				...(normalizedEmail
					? [{ primaryEmail: { equals: normalizedEmail, mode: "insensitive" } }, { secondaryEmail: { equals: normalizedEmail, mode: "insensitive" } }]
					: []),
			],
		},
		include: {
...
export function getClientOutstandingBalance(client) {
	...
	return client.invoices.reduce((sum, invoice) => {
		return sum + getInvoiceOutstandingBalance(invoice);
	}, 0);
}
...
export function getNextPaymentDate(client) {
	...
	const upcoming = [...client.invoices]
		...
		.filter((invoice) => (invoice.amount ?? 0) > getInvoicePaidAmount(invoice))
