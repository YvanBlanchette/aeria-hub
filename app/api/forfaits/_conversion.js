...
export function computeForfaitPricingRows(draft, constants) {
...
export async function createQuoteFromForfait({ source, tripId }) {
	const draft = source?.payload;
	const constants = source?.constants;
	if (!draft || typeof draft !== "object" || !constants || typeof constants !== "object") {
		return { error: "Invalid package payload.", status: 400 };
	}

	const { rows, pax } = computeForfaitPricingRows(draft, constants);
	if (rows.length === 0) {
		return { error: "No active cabin category to convert.", status: 400 };
	}
	...
	return { quote, pax, lineItems: rows.length };
}
