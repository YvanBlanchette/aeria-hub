...
/** First segment before a comma ("Cozumel, Mexico" -> "Cozumel"), stripped of a trailing "(Region)" qualifier, used for geocoding/wiki lookups. */
...
async function geocodeLocation(location) {
	...
	// Not cached: a transient failure would otherwise be stuck returning null for the full revalidate window.
...
async function getWikipediaSummary(location) {
	...
	const res = await fetch(`${WIKIPEDIA_SUMMARY_URL}/${title}`, {
		headers: { Accept: "application/json", "User-Agent": "AeriaHub/1.0 (https://aeriavoyages.com; travel-agency trip planning app)" },
		// Not cached: a transient failure (rate limit, network blip) would otherwise stick for the full revalidate window.
		cache: "no-store",
	});
	if (!res.ok) return null;
	const data = await res.json();
...
export async function getDestinationInfo(location, referenceDate) {
	...
	const [geo, overview] = await Promise.all([geocodeLocation(location).catch(() => null), getWikipediaSummary(location).catch(() => null)]);
