...
export default async function InvoicesPage({ params }) {
	...
	const invoices = await prisma.invoice.findMany({
		...
		include: { trip: { select: { name: true, payments: { where: { cancelled: false }, select: { amount: true } } } } },
	});
	...
	return (
		<div className="space-y-4">
			...
			{invoices.length === 0 ? (
				<Card>
					<CardContent className="py-10 text-center text-sm text-muted-foreground">No invoices on record for this client.</CardContent>
				</Card>
			) : (
