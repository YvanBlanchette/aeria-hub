...
import { ClientSegmentEditDialog } from "@/components/trips/client-segment-edit-dialog";
			...
			<CardContent className="flex items-start gap-3 p-4">
				...
				<div className="min-w-0 flex-1 space-y-1">
					...
					<p className="text-sm text-muted-foreground">
						...
						{segment.supplier && (
							<Link
								href={`/suppliers/${segment.supplier.id}`}
				...
				<div className="flex shrink-0 items-center gap-3">
					{canManage && segment.cost != null && <span className="text-sm tabular-nums text-muted-foreground">{formatCurrency(segment.cost)}</span>}
					{canManage && (
					...
					{canClientEdit && <ClientSegmentEditDialog segment={segment} />}
				</div>
			</CardContent>
		</Card>
	);
